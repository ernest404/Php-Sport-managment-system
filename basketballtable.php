...
<?php
  $query = "SELECT * FROM tbl_scorers INNER JOIN tbl_teamroaster ON tbl_scorers.name=tbl_teamroaster.ID WHERE tbl_teamroaster.sport = '1' ORDER by score DESC, ts ASC LIMIT 10";
  $result = mysqli_query($conn,$query) or die('Query failed: ' . mysqli_error());
  $result_length = mysqli_num_rows($result);
  for($i = 1; $i <= $result_length; $i++)
  {    
